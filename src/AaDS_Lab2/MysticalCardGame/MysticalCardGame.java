package AaDS_Lab2.MysticalCardGame;
...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> mostPowerful = firstSorcererCards.getFirst();
        SLLNode<Card> tmp = firstSorcererCards.getFirst();

        while (tmp != null) {
            if (mostPowerful.element.significance() < tmp.element.significance()) {
                mostPowerful = tmp;
            } else {
                tmp = tmp.succ;
            }
        }
        firstSorcererCards.delete(mostPowerful); // deleting the most powerful card
        tmp = secondSorcererCards.getFirst(); // starting over again
        int n = (secondSorcererCards.size() / 2) - 1; // -1 again because of getFirst starting from int i = 1;

        while (tmp != null) { //after reaching the middle point calculated by n-- then insert
            if (n-- == 0) {
                secondSorcererCards.insertAfter(mostPowerful.element, tmp); //remember it's what do we want to insert and then after what
            } else {
                tmp = tmp.succ;
            }
        }
