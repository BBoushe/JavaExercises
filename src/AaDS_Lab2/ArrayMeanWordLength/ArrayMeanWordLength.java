package AaDS_Lab2.ArrayMeanWordLength;
...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        for(int i = 0; i < n; i++) { // get the length of all words
            sum += arr.get(i).length();
        }

        double average =  sum / n;

        String currentClosestWord = arr.get(0);

        for(int i = 1; i < n; i++){
            if(Math.abs(arr.get(i).length() - average) == Math.abs(currentClosestWord.length() - average)) {
                if (arr.get(i).length() > currentClosestWord.length()) {
                    currentClosestWord = arr.get(i);
                }
            }
            else if (Math.abs(arr.get(i).length() - average) < Math.abs(currentClosestWord.length() - average)) {
                    currentClosestWord = arr.get(i);
                }
        }
